...
bool CCollisionMgr::Check_Rect(CObj * pDest, CObj * pSour, float * pX, float * pY) //px?€ pY??ì£¼ì†Œê°’ì„ ?˜ê²¨ì£¼ì–´???¨ìˆ˜ê°€ ?ë‚œ?„ì—??ë³€?˜ì— ?€?…í•œ ê°’ì„ ê³„ì† ?¸ìˆ˜?ˆë‹¤
{
	...
	if ((fCX > fWidth) && (fCY > fHeight)) // ?ì  ?¬ì´??ê±°ë¦¬ê°€ ?ê°ì²´ì˜ ë°˜ì?ë¦„ì„ ?”í•œê°’ë³´???‘ì•„ì§€ë©?ì¶©ëŒ
	{
		*pX = fCX - fWidth;
		*pY = fCY - fHeight; // ?ê°ì²´ì˜ ë°˜ì?ë¦„ì„ ?”í•œê°’ì—???ì ?¬ì´??ê±°ë¦¬ë¥?ë¹¼ì£¼ë©?ì¶©ëŒ??ê±°ë¦¬, ê¸¸ì´ê°€ ?˜ì˜¨??

		return true; // ì¶©ëŒ?ˆì„ê²½ìš° true ë°˜í™˜
	}
...
bool CCollisionMgr::Check_Sphere(CObj * pDest, CObj * pSour)
{
	...
	return fRadius > fDiagonal; // ???ì˜ ë°˜ì?ë¦„ì„ ?”í•œê°’ì´ ??ê°ì²´?¬ì´??ê¸¸ì´ë³´ë‹¤ ?¬ë‹¤ = ì¶©ëŒ
}

void CCollisionMgr::Collision_Rect(list<CObj*> _Dest, list<CObj*> _Sour) // obj?¬ì¸???€??ë¦¬ìŠ¤?¸ì•ˆ???ˆëŠ” ??ê°ì²´ê°€ ì¶©ëŒ?ˆì„??ì²˜ë¦¬?˜ëŠ” ?¨ìˆ˜
{
	RECT rc{}; // ì¶©ëŒ??ë²”ìœ„ë¥??´ëŠ” ë³€??
	...
	for (auto& Dest : _Dest)
	{
		for (auto& Sour : _Sour)
		{
			if (IntersectRect(&rc, &(Dest->Get_Rect()), &(Sour->Get_Rect())))
			{
				...
				//??ê°ì²´ê°€ ì¶©ëŒ?˜ì—ˆ?„ë•Œ trueë¥?ë°˜í™˜?œë‹¤
				//true???íƒœ = ??ê°ì²´ê°€ ì¶©ëŒ?˜ì—ˆ?¤ëŠ” ê²ƒì„ ?˜ë??˜ë?ë¡??´ë•Œ ?í•˜???‰ë™???˜ëŠ” ?¨ìˆ˜ë¥??£ì–´ì£¼ë©´?œë‹¤
				//ex) Set_Dead
...
void CCollisionMgr::Collision_RectEx(list<CObj*> _Dest, list<CObj*> _Sour)
{
	for (auto& Dest : _Dest)
	{
		for (auto & Sour : _Sour)
		{
			float fX = 0.f, fY = 0.f; // ì¶©ëŒ??ê¸¸ì´ë¡??¬ìš©??ë³€??
			//Dest ?Œë ˆ?´ì–´ Sour ëª¬ìŠ¤??
			if (Check_Rect(Dest, Sour, &fX, &fY))
			{
				//?í•˜ì¶©ëŒ
				if (fX > fY) // ì¶©ëŒ?ˆì„???˜ì˜¨ X??ê¸¸ì´ê°€ Y??ê¸¸ì´ë³´ë‹¤ ê¸¸ë‹¤ë©??í•˜ì¶©ëŒ 
				{
					if (Sour->Get_Rect().top < Dest->Get_Rect().bottom) // ê³ ì •?˜ì–´?ˆëŠ”ë¬¼ì²´??y??ê°’ì´ ?€ì§ì´??ë¬¼ì²´??y??ê°’ë³´????ê²½ìš°, ??ì¶©ëŒ
					{
						//Dest->Set_PostY(fY);
						...
						//Sour->Set_PosY(-fY); // ì¶©ëŒ??ê¸¸ì´ë§Œí¼ ?„ë¡œ ?¬ë¼ê°€ê²?-fYê°’ì„ ?£ëŠ”??
					}
					else //??ì¶©ëŒ
					{
						//Sour->Set_HpMinus();
